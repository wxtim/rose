...
class ConfigDiffDefaults(object):
    ...
    """Store default settings for themetomi.rose.config-diff command."""
...
def annotate_config_with_metadata(config, meta_config, ignore_regexes=None,
    ...
    """Add metadata to themetomi.rose.config.ConfigNode.comments attribute.

    config -- ametomi.rose.config.ConfigNode instance, containing app or
              suite data.
    meta_config -- ametomi.rose.config.ConfigNode instance, containing
                   metadata for config.
    ...
    for path in paths:
        ...
        if os.path.isdir(path):
            for filename in metomi.rose.CONFIG_NAMES:
                ...
                raisemetomi.rose.run.ConfigNotFoundError(
