...
run_pass "${TEST_KEY_BASE}" python "${SCRIPT}" $PWD/foo 'blob' 'tree'
awk '{print $1, $3, $4}' > cut_test_file < "${TEST_KEY_BASE}.out"
file_cmp "${TEST_KEY_BASE}-vs-kgo" "cut_test_file" <<__HERE__
tree  
- - $PWD/foo/baz
33188 3 $PWD/foo/bar
33188 6 $PWD/foo/baz/qux
__HERE__

# Run the script on a blob:
run_pass "${TEST_KEY_BASE}" python "${SCRIPT}" $PWD/foo/bar 'blob' 'tree'
awk '{print $1, $3, $4}' > cut_test_file < "${TEST_KEY_BASE}.out"
file_cmp "${TEST_KEY_BASE}-vs-kgo" cut_test_file <<__HERE__
blob  
33188 3 $PWD/foo/bar
